...
function detailPreview(item) {
  ...
  const states = (item.stateFlow || []).map((state) => `<span class="state-${escapeHtml(state.value)}"><b>${escapeHtml(state.label)}</b><small>${escapeHtml(state.meaning)}</small></span>`).join('');
  const transitions = (item.transitions || []).map((transition) => `<li><span>${escapeHtml(transition.from.replace('_', ' '))}</span><i>→</i><span>${escapeHtml(transition.to.replace('_', ' '))}</span>${transition.gate ? `<b>gate: ${escapeHtml(transition.gate)}</b>` : ''}</li>`).join('');
  return `<section class="detail-preview tone-${toneByLane[item.lane] || 'indigo'}"><div class="preview-header"><div><span>VALIDATED WORKFLOW METADATA VIEW</span><h3>${escapeHtml(item.recordLabel || item.name)}</h3></div><em>${escapeHtml(item.status)}</em></div><div class="preview-layout"><div class="preview-rail">${states}</div><div class="preview-work"><div class="preview-metric"><span>${escapeHtml(item.recordLabel || 'ACTIVE RECORD').toUpperCase()}</span><b>01</b><small>Independent local persistent workflow</small></div><div class="detail-fields">${fields}</div><div class="resolution-row"><span>${escapeHtml(item.resolveLabel || 'Evidence-gated closeout')}</span><b>${escapeHtml(evidence)}</b></div><ol class="transition-list">${transitions}</ol></div></div></section>`;
}
